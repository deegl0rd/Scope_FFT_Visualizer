...
volatile uint16_t FFT_BUFFER[FFT_BUFFER_SIZE];
volatile int IsADCHalfReady = 0b0;
volatile int IsADCFullReady = 0b0;
volatile int IsUserButtonPressed = 0b0;
volatile int IsFFTMode = 0b0;
volatile int IsFFTReady = 0b0;
volatile int FFTwriteIndex = 0;
/* USER CODE END PV */
...
int  writeIndex = 0;
void PushToFFTBuf(uint16_t* src, int len)
{
    for(int i = 0; i < len; i++)
    {
    	FFT_BUFFER[FFTwriteIndex++] = src[i];

        if(FFTwriteIndex >= FFT_BUFFER_SIZE)
        {
        	FFTwriteIndex = 0;
            IsFFTReady = 0b1;
        }
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef* hadc)
{
	if (hadc->Instance == ADC1)
	{
		...
		if (IsFFTMode && !IsFFTReady)
			PushToFFTBuf((uint16_t*) &ADC_BUFFER[0], ADC_BUFFER_SIZE / 2);
	}
}
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc)
{
	if (hadc->Instance == ADC1)
	{
		...
		if (IsFFTMode && !IsFFTReady)
			PushToFFTBuf((uint16_t*) &ADC_BUFFER[ADC_BUFFER_SIZE / 2], ADC_BUFFER_SIZE / 2);
	}
}

void HAL_ADC_ErrorCallback(ADC_HandleTypeDef *hadc)
{
	//volatile uint32_t err = HAL_ADC_GetError(&hadc1);
